...
namespace scheidingsdesk_document_generator.Services.DocumentGeneration.Processors.PlaceholderBuilders
{
    ...
    public class ConvenantPlaceholderBuilder : BasePlaceholderBuilder
    {
        ...
        /// For convenant: de man/de vrouw when anonymous, de vader/de moeder when named.
        /// </summary>
        private void BuildPartijAanduidingPlaceholders(Dictionary<string, string> replacements, DossierData data)
        {
            ...
            // Determine aanduidingen based on gender and anonymity
            string partij1Aanduiding, partij2Aanduiding;

            if (isAnoniem)
            {
                // Anonymous: use de man / de vrouw
                partij1Aanduiding = GetGeslachtAanduiding(partij1?.Geslacht, isAnoniem);
                partij2Aanduiding = GetGeslachtAanduiding(partij2?.Geslacht, isAnoniem);
            }
            else
            {
                // Named: use de vader / de moeder
                partij1Aanduiding = GetGeslachtAanduiding(partij1?.Geslacht, false);
                partij2Aanduiding = GetGeslachtAanduiding(partij2?.Geslacht, false);
            }
        ...
        private string GetGeslachtAanduiding(string? geslacht, bool isAnoniem)
        {
